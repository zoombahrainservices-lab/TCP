...
export async function getStudentXP(studentId: string): Promise<{
  ...
  if (XP_CONFIG.XP_SYSTEM_VERSION === 'v2') {
    // Fetch each event type in parallel with filtered queries
    // This is more efficient than fetching all events and filtering in JavaScript
    const [phaseResult, missionResult, zoneResult] = await Promise.all([
      supabase
        .from('xp_events')
        .select('xp_amount')
        .eq('user_id', studentId)
      ...
      supabase
        ...
        .select('xp_amount')
        .eq('user_id', studentId)
        .eq('event_type', 'MISSION_COMPLETE'),
      supabase
        .from('xp_events')
        .select('xp_amount')
        .eq('user_id', studentId)
        .eq('event_type', 'ZONE_COMPLETE')
    ])
