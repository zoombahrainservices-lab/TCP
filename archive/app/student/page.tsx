...
export default async function StudentPage() {
  ...
  // Start chapter fetch immediately in parallel while we process zones data
  const chapterPromise = progress.suggestedChapterId
    ? getChapter(progress.suggestedChapterId).catch(() => null)
  ...
  // Map zones to the new component format (runs while chapter loads)
  ...
  // Await chapter fetch (should be ready or nearly ready by now)
  const suggestedChapter = await chapterPromise
  const nextMissionNumber = suggestedChapter?.chapter_number
