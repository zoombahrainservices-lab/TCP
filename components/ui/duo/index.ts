/**
 * Duolingo-Style UI Components for TCP
 * 
 * These components follow Duolingo's friendly, colorful design patterns
 * while using TCP's brand colors (Amber, Blue, Red, Charcoal, Off-white)
 */

export { DuoCard } from '../DuoCard'
export { DuoButton } from '../DuoButton'
export { ProgressCircle } from '../ProgressCircle'
export { DuoCharacter } from '../DuoCharacter'

// Export types if needed
export type { DuoCardProps } from '../DuoCard'
export type { DuoButtonProps } from '../DuoButton'
export type { ProgressCircleProps } from '../ProgressCircle'
export type { DuoCharacterProps } from '../DuoCharacter'
