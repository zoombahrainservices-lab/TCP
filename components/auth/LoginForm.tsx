...
export default function LoginForm() {
  ...
  const handleEmailLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!res.ok) {
        setError(data.error || SIGN_IN_ERROR)
        setLoading(false)
        return
      }
    ...
    <div className="w-full max-w-md space-y-6">
      <form onSubmit={handleEmailLogin} className="space-y-4">
        ...
        <div className="w-full">
          <div className="flex flex-wrap items-center justify-between gap-x-2 gap-y-1 mb-1">
            ...
            <Link
              ...
              className="text-sm font-medium text-[#0770C4] dark:text-[#51BFE3] hover:underline shrink-0"
            >
              Forgot password?
            </Link>
          </div>
        ...
        <div className="flex items-start gap-3 pt-1">
          <input
            ...
            disabled={loading}
            className="mt-1 h-4 w-4 rounded border-gray-300 text-[#0770C4] focus:ring-[#0770C4] dark:border-gray-600 dark:bg-gray-800 dark:focus:ring-[#51BFE3]"
          />
          <div className="flex-1 min-w-0">
            <label htmlFor="keep-signed-in" className="text-sm font-medium text-gray-700 dark:text-gray-300 cursor-pointer">
              Keep me signed in
            </label>
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-0.5 leading-snug">
              Only use this on your own device.
            </p>
          </div>
        </div>
      ...
      <div className="relative">
        ...
        <div className="relative flex justify-center text-sm">
          <span className="px-2 bg-gray-50 dark:bg-[#142A4A] text-gray-500 dark:text-gray-400">
            Or continue with
          </span>
        </div>
      </div>
