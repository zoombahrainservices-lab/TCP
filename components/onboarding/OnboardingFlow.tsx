...
export function OnboardingFlow() {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gradient-to-br from-white via-gray-50/50 to-white dark:from-[#0a1628] dark:via-[#142A4A] dark:to-[#0a1628] overflow-hidden">
      {/* Progress bar */}
      <div className="absolute top-0 left-0 right-0 h-3 bg-gray-200 dark:bg-gray-700">
        <motion.div
      ...
      <button
        onClick={handleClose}
        className="absolute top-6 right-6 p-2.5 rounded-full bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm hover:bg-white dark:hover:bg-gray-700 transition-all shadow-lg z-10"
      >
      ...
      <div className="absolute bottom-8 right-8 z-10 opacity-30">
        <Image src="/TCP-Logo.svg" alt="TCP" width={200} height={60} className="w-48 h-auto" />
      </div>
          ...
          <AnimatePresence mode="wait">
            ...
            {currentStep === 0 && (
              ...
              >
                ...
                <div className="hidden lg:grid grid-cols-3 gap-3 max-w-4xl mx-auto">
                  {focusAreas.map((area) => (
                    <button
                      key={area.id}
                      onClick={() => setSelectedOption(area.id)}
                      className={`relative group rounded-xl p-3 transition-all cursor-pointer ${
                    ...
                    >
                      <div className={`w-full aspect-square rounded-lg mb-2 flex items-center justify-center overflow-hidden relative ${
                ...
                {/* Carousel with arrows for smaller screens/zoomed */}
                <div className="lg:hidden relative flex items-center justify-center max-w-4xl mx-auto">
                  {/* Left Arrow */}
                  <button
                    onClick={handlePrevCard}
                    className="absolute left-0 z-10 p-3 rounded-full bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all hover:scale-110 border border-gray-200 dark:border-gray-700"
                  ...
                  <div className="w-full max-w-md px-16">
                    <AnimatePresence mode="wait">
                      <motion.button
                        ...
                        transition={{ duration: 0.3 }}
                        onClick={() => setSelectedOption(focusAreas[currentCardIndex].id)}
                        className={`w-full group rounded-xl p-4 transition-all cursor-pointer ${
                          selectedOption === focusAreas[currentCardIndex].id
                            ? `bg-gradient-to-br ${focusAreas[currentCardIndex].gradient} text-white shadow-2xl scale-105`
                            : 'bg-white dark:bg-[#1a3456] border-2 border-gray-200 dark:border-gray-700 hover:border-[#0073ba] dark:hover:border-[#4bc4dc] shadow-lg hover:shadow-xl hover:scale-105'
                        }`}
                      >
                        <div className={`w-full aspect-square rounded-lg mb-3 flex items-center justify-center overflow-hidden relative ${
                          selectedOption === focusAreas[currentCardIndex].id ? 'bg-white/15' : 'bg-gray-50 dark:bg-gray-800'
                        }`}>
                          {/* Default image */}
                          <Image 
                            src={focusAreas[currentCardIndex].image} 
                            alt={focusAreas[currentCardIndex].title}
                            width={300}
                            height={300}
                            quality={100}
                            priority
                          ...
                          {/* Hover image */}
                          <Image 
                            src={focusAreas[currentCardIndex].hoverImage} 
                            alt={`${focusAreas[currentCardIndex].title} hover`}
                            width={300}
                            height={300}
                            quality={100}
                            priority
                        ...
                        <h3 className={`text-lg md:text-xl font-bold mb-2 ${
                          selectedOption === focusAreas[currentCardIndex].id ? 'text-white' : 'text-gray-900 dark:text-white'
                        }`}>
                          {focusAreas[currentCardIndex].title}
                        </h3>
                        <p className={`text-sm ${
                          selectedOption === focusAreas[currentCardIndex].id ? 'text-white/90' : 'text-gray-600 dark:text-gray-400'
                        }`}>
                          {focusAreas[currentCardIndex].description}
                        </p>
                        {selectedOption === focusAreas[currentCardIndex].id && (
                  ...
                  <button
                    ...
                    className="absolute right-0 z-10 p-3 rounded-full bg-white dark:bg-gray-800 shadow-lg hover:shadow-xl transition-all hover:scale-110 border border-gray-200 dark:border-gray-700"
            ...
            {currentStep === 2 && generatedPromises && (
              ...
              >
                ...
                <div className="grid gap-6">
                  ...
                  <div className="flex items-start gap-6 p-6 bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700">
              ...
              >
                ...
                <div className="grid md:grid-cols-2 gap-6 max-w-5xl mx-auto scale-90">
                  ...
                  >
                    ...
                    <div className="h-96 bg-gradient-to-br from-[var(--color-blue)]/10 to-[var(--color-blue)]/5 flex items-center justify-center overflow-hidden">
                      <Image 
                  ...
                  >
                    {/* Illustration - top section */}
                    <div className="h-96 bg-gradient-to-br from-[var(--color-amber)]/10 to-[var(--color-amber)]/5 flex items-center justify-center overflow-hidden relative">
                      {/* Badge */}
      ...
      <div className="absolute bottom-8 left-0 right-0 flex items-center justify-center gap-4 px-6">
        ...
        <div className="relative">
          ...
          >
            <Info className="w-4 h-4 text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-400 transition-colors" />
          </div>
          ...
          <AnimatePresence>
            {showTooltip && (
              <motion.div
                ...
                exit={{ opacity: 0, y: 10 }}
                className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 w-72 p-3 bg-white dark:bg-gray-800 rounded-lg shadow-xl border border-gray-200 dark:border-gray-700"
              >
                <p className="text-xs text-gray-700 dark:text-gray-300 leading-relaxed">
                  <strong>Data Collection Notice:</strong> The information you provide is collected solely to personalize your learning experience. This data enables our platform to recommend tailored activities and learning paths that align with your specific communication development goals. Your responses are stored securely and processed in accordance with applicable data protection regulations.
                </p>
              </motion.div>
            )}
          </AnimatePresence>
        ...
        <motion.button
          ...
          className={`px-8 py-3 rounded-2xl font-bold text-sm uppercase tracking-wide transition-all ${
