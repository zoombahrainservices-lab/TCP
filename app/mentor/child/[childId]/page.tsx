...
export default function StudentProfilePage() {
  ...
  const handleViewDay = async (dayNumber: number) => {
    ...
    try {
      const sub = await getChildDaySubmission(mentorId, childId, dayNumber)
      setSubmission(sub)
    } catch (error) {
  ...
  if (!progress) {
    return (
      <div className="max-w-3xl mx-auto px-4 py-16 text-center">
        <p className="text-red-600 text-lg mb-4">Failed to load student progress</p>
        <Button onClick={() => router.push('/mentor')}>Back to Dashboard</Button>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      {/* Header */}
      <div className="mb-8">
        <Link href="/mentor" className="text-blue-600 hover:text-blue-700 mb-4 inline-block">
          ← Back to Dashboard
        </Link>
        <div className="flex items-center justify-between">
          ...
          <Link href={`/mentor/child/${childId}/report`}>
